/**
 * 自定义source组件入口
 * 请只处理SourceRules.process(events)即可
 */
public class CustomizeSource extends AbstractSource
        implements Configurable, EventDrivenSource, BatchSizeSupported {

    private static final Logger logger = LoggerFactory.getLogger(CustomizeSource.class);
    ...
    public synchronized void start() {
        ...
        super.start();
        logger.debug("CustomizeSource source started");
        sourceCounter.start();
    }
        ...
        public void run() {
            ...
            try {
                while (!Thread.interrupted()) {
                    //----改造代码
                    //logger.info("开始读取一批数据  ->" + LocalDateTime.now());
                    //----改造代码
                    readingEvents = true;
                    ...
                    //新建一个events进行存储
                    List<Event> processEvents = Lists.newLinkedList();

                    for (Event event : events) {
                        //大于1024就排除此行
                        if(event.getBody().length>1024){
                            continue;
                        }
                        //根据规则预清洗event
                        Event processEvent = SourceRules.processByPythonScript(event);
                        if (!(processEvent == null)) {
                            processEvents.add(event);
                        }
                    }

                    //将所有的events后面加一个空event,防止出现空events报错
                    if (processEvents.size() == 0) {
                        processEvents.add(EventBuilder.withBody(new byte[0]));
                    }

                    //----改造代码----

                    sourceCounter.addToEventReceivedCount(processEvents.size());
                    sourceCounter.incrementAppendBatchReceivedCount();

                    try {

                        getChannelProcessor().processEventBatch(processEvents);
                        reader.commit();
                ...
                logger.error("FATAL: " + CustomizeSource.this.toString() + ": " +
                        "Uncaught exception in CustomizeSource thread. " +
